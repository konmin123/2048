...
class Game:
    ...
    def clear_fild(self) -> None:
        """Метод создающий новое, чистое поле."""
        self.field = [[0 for _ in range(4)] for _ in range(4)]

    def check_add(self) -> bool:
        """Метод проверяющий наличие доступных для хода клеток."""
    ...
    def check_win(self) -> bool:
        """Метод проверяющий победил ли игрок."""
        for line in self.field:
            if 2048 in line:
                return True
        return False
    ...
    def add_two(self) -> None:
        """Метод создающий список возможных клеток для добавления двойки и добавляющий её."""
        list_of_moves = []
    ...
    def tern_left(self, x=1) -> None:
        """Метод осуществляющий поворот поля против часовой стрелки на 90 градусов x раз."""
        for _ in range(x):
            new_field = [[], [], [], []]
            for line in self.field:
                for index_value, value in enumerate(line):
    ...
    def merge_left(self) -> None:
        """Метод складывающий одинаковые значения не равные 0, в отсортированных списках."""
        self.sort_for_merge()
    ...
    def merge_right(self) -> None:
        """Метод правого слияния через повороты поля и левое слияние."""
        self.tern_left(2)
        self.merge_left()
        self.tern_left(2)

    def merge_up(self) -> None:
        """Метод верхнего слияния через повороты поля и левое слияние."""
        self.tern_left()
        self.merge_left()
        self.tern_left(3)

    def merge_down(self) -> None:
        """Метод нижнего слияния через повороты поля и левое слияние."""
        self.tern_left(3)
        self.merge_left()
        self.tern_left()

    def sort_for_merge(self) -> None:
        """Метод создающий отсортированную копию поля и заменяющий на неё поле."""
        sorted_field = [[], [], [], []]
        for index_line, line in enumerate(self.field):
            count = 0
            for value in line:
    ...
    def input_play(self, input_) -> None:
        """Метод вызывающий слияние в сторону, выбранную игроком."""
        if input_ == 'w':
            self.merge_up()
        if input_ == 's':
            self.merge_down()
    ...
    def show_score(self) -> int:
        """Метод возвращающий максимальное значение на игровом поле."""
        max_value = 0
        for line in self.field:
            for value in line:
                if value > max_value:
